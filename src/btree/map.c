...
nativeBTreeForEach(gpointer key, gpointer val, gpointer data)
{
  ...
  NAPI_CALL(env, false, napi_get_named_property(env, esObject, KEY, &esKey));
...
esGet(napi_env env, napi_callback_info cbInfo)
{
  ...
  if (lookupResult == NULL) {
    NAPI_CALL(env, false, napi_get_undefined(env, &result));
  }
  else {
    NAPI_CALL(env, false,
              napi_get_reference_value(env, lookupResult->esKeyValue, &result));

    NAPI_CALL(env, false, napi_get_named_property(env, result, VALUE, &result));
  }
...
esDelete(napi_env env, napi_callback_info cbInfo)
{
  ...
  NAPI_CALL(env, false, napi_create_object(env, &searchBox));

  NAPI_CALL(env, false, napi_set_named_property(env, searchBox, KEY, arg));

  NAPI_CALL(env, false,
            napi_create_reference(env, searchBox, 0, &searchBoxRef));

  BTreeNode_t searchNode = {searchBoxRef, NULL};

  gboolean found = g_tree_remove(bTree->nativeTree, &searchNode);

  NAPI_CALL(env, false, napi_get_boolean(env, found, &result));
...
{
  napi_value esThis;
  size_t argc = 2, expectedArgc = argc;
  ...
  for (size_t i = argc; i < expectedArgc; i++) {
    NAPI_CALL(env, false, napi_get_undefined(env, &argv[i]));
  }
  ...
  NAPI_CALL(env, false, napi_set_named_property(env, box, KEY, key));
  NAPI_CALL(env, false, napi_set_named_property(env, box, VALUE, value));
...
esSize(napi_env env, napi_callback_info cbInfo)
{
  ...
  NAPI_CALL(env, false, napi_create_int64(env, (int64_t) nativeSize, &esSize));
...
esClear(napi_env env, napi_callback_info cbInfo)
{
  ...
  g_tree_remove_all(bTree->nativeTree);
#else
  reallocGTree(bTree);
#endif
...
esHas(napi_env env, napi_callback_info cbInfo)
{
  ...
  NAPI_CALL(env, false, napi_set_named_property(env, box, KEY, argv[0]));

  NAPI_CALL(env, false, napi_create_reference(env, box, 0, &boxRef));

  BTreeNode_t searchNode = {boxRef, NULL};
...
esForeach(napi_env env, napi_callback_info cbInfo)
{
  ...
  ForEachContext_t ctxt = {esThis, callback, cbThis, 0, bTree, NULL};

  g_tree_foreach(bTree->nativeTree, nativeBTreeForEach, &ctxt);

  NAPI_CALL(env, false, napi_get_undefined(env, &undefined));
