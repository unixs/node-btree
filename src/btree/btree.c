...
btree_export(napi_env env)
{
  ...
  napi_value symbolToStringTag;
  NAPI_GLOBAL_SYM(env, "toStringTag", symbolToStringTag);

  // Instance props
  napi_property_descriptor esBTreeProps[] = {
      DEF_V_PROP(symbolToStringTag, esToStringTag),
...
#ifdef HAS_GTREE_NODE
      DEF_METHOD_V_WITH_DATA(symbolIterator, esGenerator,
                             (void *) iteratorResultDefaultCb),
      DEF_METHOD_WITH_DATA("entries", esGenerator,
                           (void *) iteratorResultDefaultCb),
      DEF_METHOD_WITH_DATA("values", esGenerator,
...
// Extra methods
  ...
  // Static props
  napi_property_descriptor staticProps[] = {DEF_METHOD("from", esStaticFrom)};

  NAPI_CALL(env, false,
            napi_define_class(env, ES_CLASS_NAME, NAPI_AUTO_LENGTH,
                              esConstructor, NULL,
                              (sizeof(esBTreeProps) / sizeof(esBTreeProps[0])),
                              esBTreeProps, &esBTreeClass));

  NAPI_CALL(env, false,
            napi_create_reference(env, esBTreeClass, 1, &btreeConstructorRef));

  NAPI_CALL(env, false,
            napi_define_properties(env, esBTreeClass,
                                   (sizeof(staticProps) / sizeof(staticProps)),
                                   staticProps));

  napi_property_descriptor descriptor = {
      ES_CLASS_NAME, NULL, NULL, NULL, NULL, esBTreeClass, napi_default, NULL};
