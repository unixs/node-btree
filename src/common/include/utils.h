...
#define DEF_METHOD_V_WITH_DATA(name, func, data)                               \
  {NULL, name, func, NULL, NULL, NULL, napi_default, data}

#define DEF_METHOD_WITH_DATA(name, func, data)                                 \
  {name, NULL, func, NULL, NULL, NULL, napi_default, data}

#define DEF_PROP(name, cb)                                                     \
  {name, NULL, NULL, cb, NULL, NULL, napi_default, NULL}

#define DEF_V_PROP(name, cb)                                                   \
  {NULL, name, NULL, cb, NULL, NULL, napi_default, NULL}
...
#define CHECK_ARGC(expected, msg)                                              \
  if (argc < expected) {                                                       \
    NAPI_CALL(env, false, napi_throw_error(env, NULL, msg));                   \
    return NULL;                                                               \
  }
 ...
 * TODO: Replace with function with static var?
 */
#define NAPI_GLOBAL_SYM(env, name, napiVar)                                    \
  {                                                                            \
    ...
    NAPI_CALL(env, false,                                                      \
              napi_get_named_property(env, Symbol, name, &napiVar));           \
  }
