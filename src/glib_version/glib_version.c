...
esGlibVersionFull(napi_env env, napi_callback_info cbInfo)
{
  ...
  size_t versionStrSize =
      snprintf(fullVersion, sizeof(fullVersion), "%u.%u.%u", GLIB_MAJOR_VERSION,
...
esGlibVersionMajor(napi_env env, napi_callback_info cbInfo)
{
  ...
  NAPI_CALL(env, false, napi_create_int64(env, GLIB_MAJOR_VERSION, &esMajor));
...
esGlibVersionMinor(napi_env env, napi_callback_info cbInfo)
{
  ...
  NAPI_CALL(env, false, napi_create_int64(env, GLIB_MINOR_VERSION, &esMinor));
...
esGlibVersionPatch(napi_env env, napi_callback_info cbInfo)
{
  ...
  NAPI_CALL(env, false, napi_create_int64(env, GLIB_MICRO_VERSION, &esPatch));
...
esHasGTreeNode(napi_env env, napi_callback_info cbInfo)
{
  ...
  NAPI_CALL(env, false, napi_get_boolean(env, c_result, &result));
...
glib_version_export(napi_env env)
{
  ...
  napi_property_descriptor descriptor = {"GLIB_VERSION", NULL, NULL,
                                         NULL,           NULL, esGlibVersion,
                                         napi_default,   NULL};
